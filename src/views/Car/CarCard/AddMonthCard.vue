        ...
        <div class="form">
          <el-form label-width="100px" ref="cardInfoForm" :model="cardInfoForm" :rules="cardRules">
            <el-form-item label="车主姓名" prop="personName">
              <el-input v-model="cardInfoForm.personName"/>
            </el-form-item>
            <el-form-item label="联系方式" prop="phoneNumber">
              <el-input v-model="cardInfoForm.phoneNumber"/>
            </el-form-item>
            <el-form-item label="车辆号码" prop="carNumber">
              <el-input v-model="cardInfoForm.carNumber"/>
            </el-form-item>
            <el-form-item label="车辆品牌" prop="carBrand">
              <el-input v-model="cardInfoForm.carBrand"/>
            </el-form-item>
          </el-form>
        </div>
      </div>
...
<script>
import { addCardApi, getEditCardDetailApi, updateCardApi } from '@/api/card'
...
export default {
  ...
  methods: {
    confirmAdd() {
      this.$refs.cardInfoForm.validate(flag => {
        ...
        this.$refs.feeForm.validate(async flag => {
          ...
          if (this.id) {
            // 编辑月卡
            await updateCardApi(requestData)
            this.$message.success('修改月卡成功')
          } else {
    ...
    async getCardDetail() {
      const res = await getEditCardDetailApi(this.id)
